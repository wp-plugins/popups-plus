...
<table class="ppl_table widefat" id="ppl_ruñes">
	<tbody>
	<tr>
		<td class="label">
			<label for="post_type"><?php _e("Rules", $this->plugin_slug ); ?></label>
			<p class="description"><?php _e("Create rules to determine where the popup will show", $this->plugin_slug ); ?></p>
		</td>
		<td>
			...
			<table class="ppl_table widefat">
				<tbody>
					...
					<td class="param"><?php 
						...
						$choices = apply_filters( 'ppl/metaboxes/rule_types', $choices );
						...
						$args = array(
							'group_id' 	=> $group_id,
							'rule_id'	=> $rule_id,
							'name'		=> 'ppl_rules[' . $group_id . '][' . $rule_id . '][param]',
							'value' 	=> $rule['param']
						);
						
						ppl_Helper::print_select( $choices, $args );
					...
					<td class="operator"><?php 	
						...
						// allow custom rules rules
						$choices = apply_filters( 'ppl/metaboxes/rule_operators', $choices );
						
						$args = array(
							'group_id' 	=> $group_id,
							'rule_id'	=> $rule_id,
							'name'		=> 'ppl_rules[' . $group_id . '][' . $rule_id . '][operator]',
							'value' 	=> $rule['operator']
					...
					<td class="value"><?php 
						$args = array(
							...
							'name'			=> 'ppl_rules[' . $group_id . '][' . $rule_id . '][value]',
							'param'			=> $rule['param']
						);
						ppl_Helper::ajax_render_rules( $args ); 
