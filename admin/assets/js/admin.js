...
ppl_ADMIN = (function ( $ ) {
	...
	$(document).ready(function(){
		...
		ppl.rules.init();
        var color_field = $('#ppl-options input.ppl-color-field');
		...
		$("#ppl-options :input").not(".ppl-color-field").change(applyStyles);

		//Toogle trigger boxes on init
		checkTriggerMethod( $("#ppl_trigger").val() );
		
		//Toogle trigger boxes on change
		$("#ppl_trigger").change(function(){
	...
	function applyStyles() 
	{
        ...
        $editor.trigger('ppl_tinymce_init');
		$editor.css({
			'background': '#9C9B9B;'
		});

        // if there is no optin mode load defaults
        if (typeof pplp_js == "undefined" || $('#ppl_optin').val() == '') {
            // remove any field that could be there after deactivating premium version
            $editor.find(".ppl-fields-container").remove();
            $editor.find("#tinymce").css({
                'padding': '25px',
                'background-color': getColor($("#ppl-background-color")),
                'border-color': getColor($("#ppl-border-color")),
                'border-width': getPxValue($("#ppl-border-width")),
                'border-style': 'solid',
                'width': $("#ppl-width").val(),
                'color': getColor($("#ppl-color")),
                'height': 'auto',
                'min-width': '200px',
                'max-width': '100%',
                'margin': '8px auto 0;'
	...
	*/
	...
	ppl.rules = {
		...
		init : function(){
			...
			_this.$el.on('change', '.param select', function(){
				...
				var $tr = $(this).closest('tr'),
					...
					ajax_data = {
						'action' 	: "ppl/field_group/render_rules",
						'nonce' 	: ppl_js.nonce,
						'rule_id' 	: rule_id,
						'group_id' 	: group_id,
						'value' 	: '',
				...
				// add loading gif
				var div = $('<div class="ppl-loading"><img src="'+ppl_js.admin_url+'/images/wpspin_light.gif"/> </div>');
		...
		add_group : function(){
			...
			// update h4
			$group2.find('h4').text( ppl_js.l10n.or );
